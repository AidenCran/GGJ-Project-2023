...
public class TimerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        firstColour.DOColor(endColor, FirstPlaceTime).SetEase(Ease.InQuad).onComplete += () => OnFirstPlaceTimeout?.Invoke();
        
        secondPlaceSlider.DOValue(secondPlaceSlider.maxValue, SecondPlaceTime).SetEase(Ease.InQuad);
        secondColour.DOColor(endColor, SecondPlaceTime).SetEase(Ease.InQuad).onComplete += () => OnSecondPlaceTimeout?.Invoke();

        thirdPlaceSlider.DOValue(thirdPlaceSlider.maxValue, ThirdPlaceTime).SetEase(Ease.InQuad);
        thirdColour.DOColor(endColor, ThirdPlaceTime).SetEase(Ease.InQuad).onComplete += () => OnThirdPlaceTimeout?.Invoke();
